...
A shared library object intended to add a functionality to an afb-binder
instance.
...
Session identifier generated by afb-binder are UUIDs.
A client can present its own session id.

Internally, afb-binder offers a mechanism to attach data to sessions.
...
At start, afb-binder get an initial token.  
...
When the token is renewed, afb-binder sends the new token to the client.

Tokens generated by afb-binder are UUIDs.
